...
def process_gridsearch(directory, SG_ratio_list, monomer_addition_list, SS_list, SG_list, GG_list):

    maximum_num_monomers = 500
    minimum_num_monomers = 10

    id = 0 

    gridsearch = {}
    for sgratio in SG_ratio_list:
        dir = directory + "/sg_" + str(sgratio)+ "/Gridsearch"
        if os.path.exists(dir):
            for i in monomer_addition_list:
                dir =  directory + "/sg_" + str(sgratio) + "/Gridsearch/monomer_addition__" + str(i)
                if os.path.exists(dir):
                    for j in SS_list:
                        dir = directory + "/sg_" + str(sgratio) +   "/Gridsearch/monomer_addition__" + str(i) + "/SS__" + str(j)
                        if os.path.exists(dir):

                            for k in SG_list:
                                dir =  directory + "/sg_" + str(sgratio) +  "/Gridsearch/monomer_addition__" + str(i) + "/SS__" + str(j) + "/SG__" + str(k)

                                if os.path.exists(dir):
                                    for h in GG_list:
                                        import_directory =  directory + "/sg_" + str(sgratio) +   "/Gridsearch/monomer_addition__" + str(i) + "/SS__" + str(j) + "/SG__" + str(k) + "/GG__" + str(h) + "/library.json"
                                        if os.path.exists(dir):
                                            print(dir)
                                            #import_directory = dir + "/library.json"
                                            ids, lib = import_library(import_directory, "ligninkmc_")
                                            bond_distribution = calculate_avg_bond_distribution(lib)


                                            dict = {"id": id,
                                            "bonds": bond_distribution,
                                            "kinetic_params":{
                                                    "min_monomers": minimum_num_monomers,
                                                    "max_monomers": maximum_num_monomers,
                                                    "sg_ratio": sgratio,
                                                    "monomer_addition_rate": i,
                                                    "scaling_params":{
                                                        "GG": h,
                                                        "SS": j,
                                                        "SG": k
                                    ...
                                    new_dict_string = "gridsearch_" + str(id)
                                    #print(bond_distribution)
                                    gridsearch[new_dict_string] = dict
                                    id += 1
    # Write to a file with the simulated conditions
    # Specify the file path
    file_path = directory + "/gridsearch.json"

    # Write the list of dictionaries to the file
    with open(file_path, 'w') as json_file:
        json.dump(gridsearch, json_file, indent=2, default=convert_int64)
